var RENDERER = {
    ...
    setParameters: function () {
        this.$container = document.getElementById("jsi-fireworks-container");
        ...
        canvasFireworks.width = this.width;
        canvasFireworks.height = this.height;
        this.$container.appendChild(canvasFireworks);
        this.contextFireworks = canvasFireworks.getContext("2d");

        // Canvas pour les branches/étoiles
        let canvasTwigs = document.createElement("canvas");
        canvasTwigs.width = this.width;
        canvasTwigs.height = this.height;
        this.$container.appendChild(canvasTwigs);
        this.contextTwigs = canvasTwigs.getContext("2d");
    ...
    render: function () {
        ...
        if (--this.leafInterval == 0) {
            ...
            this.leafInterval =
                this.getRandomValue(this.LEAF_INTERVAL_RANGE) | 0;
        }
    ...
    render: function (context) {
        switch (this.status) {
            ...
            case 2:
                ...
                for (
                    var i = 0, length = this.particles.length;
                    i < length;
                    i++
                ) {
...
PARTICLE.prototype = {
    ...
    init: function () {
        var radian = Math.PI * 2 * Math.random(),
            velocity =
                (1 - Math.pow(Math.random(), 6)) * this.VELOCITY_RANGE.max,
            rate = Math.random();
